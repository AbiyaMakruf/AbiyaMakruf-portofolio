        ...
        <div class="mx-auto max-w-7xl px-6">
            ...
            <div class="mb-12 flex flex-wrap gap-2">
                <a href="{{ route('projects.index', ['category' => 'All']) }}" 
                   class="rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 {{ request('category', 'All') === 'All' ? 'bg-gradient-to-r from-primary-500 to-accent-500 text-white shadow-lg shadow-primary-500/30 hover:shadow-xl hover:shadow-primary-500/40 hover:scale-105' : 'bg-white text-slate-600 hover:bg-gradient-to-r hover:from-primary-50 hover:to-accent-50 hover:text-primary-600 hover:scale-105 border border-slate-200' }}">
                    All
                </a>
                @foreach($categories as $category)
                    <a href="{{ route('projects.index', ['category' => $category]) }}" 
                       class="rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 {{ request('category') === $category ? 'bg-gradient-to-r from-primary-500 to-accent-500 text-white shadow-lg shadow-primary-500/30 hover:shadow-xl hover:shadow-primary-500/40 hover:scale-105' : 'bg-white text-slate-600 hover:bg-gradient-to-r hover:from-primary-50 hover:to-accent-50 hover:text-primary-600 hover:scale-105 border border-slate-200' }}">
                        {{ $category }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="grid gap-8 md:grid-cols-3">
                @forelse($projects as $project)
                    <div class="group overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-lg transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary-500/20 hover:border-primary-200 hover:scale-[1.02] flex flex-col">
                        <a href="{{ route('projects.show', $project) }}" class="block">
                            <div class="aspect-video w-full bg-slate-100 overflow-hidden">
                                @if($project->thumbnail_path)
                                    <img src="{{ $project->thumbnail_path }}" alt="{{ $project->title }}" class="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-110 group-hover:rotate-1">
                                @else
                        ...
                        <div class="p-6 flex-1 flex flex-col gap-3">
                            ...
                            <div class="flex flex-wrap gap-2 pt-2">
                                @if($project->github_url)
                                    <a href="{{ $project->github_url }}" target="_blank" class="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-primary-500 hover:text-white hover:bg-gradient-to-r hover:from-primary-500 hover:to-accent-500 hover:shadow-lg hover:shadow-primary-500/30 transition-all duration-300 hover:scale-105">
                                ...
                                @if($project->website_url)
                                    <a href="{{ $project->website_url }}" target="_blank" class="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-primary-500 hover:text-white hover:bg-gradient-to-r hover:from-primary-500 hover:to-accent-500 hover:shadow-lg hover:shadow-primary-500/30 transition-all duration-300 hover:scale-105">
